...
if 'gt_ref' in config and config['gt_ref']:
    rule vgcall:
        ...
        shell:
            ...
            vg call -t {threads} -k {input.pack} -aAz $RPATHS -s {wildcards.sample} -c {wildcards.minlen} {input.gbz} | pigz -p 8 > {output}
            """
else:
    rule vgcall:
        input: 
        ...
        shell:
            ...
            vg call -t {threads} -k {input.pack} -Az $RPATHS -s {wildcards.sample} -c {wildcards.minlen} {input.gbz} | pigz -p 8 > {output}
            """
